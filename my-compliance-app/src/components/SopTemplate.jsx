  ...
  return (
    <div className={styles.sopModalOverlay}>
      <div className={`${styles.sopContainer} ${borderRadius}`}>
        ...
        {/* Innehållshantering */}
        {/* Ersätt hela Innehållshantering-blocket med detta: */}
<div className="flex-1 overflow-y-auto p-12 bg-white">
  <div className="max-w-none prose">
    {content && content.split('\n').map((line, i) => {
      // .trim() tar bort alla mellanslag före och efter texten på raden
      const cleanLine = line.trim(); 

      if (cleanLine.startsWith('# ')) {
        return <h1 key={i} className={styles.contentHeading1}>{cleanLine.replace('# ', '')}</h1>;
      }
      if (cleanLine.startsWith('## ')) {
        return <h2 key={i} className={styles.contentHeading2}>{cleanLine.replace('## ', '')}</h2>;
      }
      if (cleanLine.startsWith('- ')) {
        return <li key={i} className="ml-4 mb-2 border-l-4 border-slate-200 pl-4 font-bold italic">{cleanLine.replace('- ', '')}</li>;
      }
      
      return cleanLine !== '' ? (
        <p key={i} className="mb-4 text-sm font-medium">{cleanLine}</p>
      ) : (
